...
const UpdateProject = () => {
  ...
  const fieldConfig = {
    ...
    2: { required: true, readOnly: false, type: "textarea" },
    3: { required: false, readOnly: false, type: "text" },
    4: { required: true, readOnly: true, type: "text" },
  };

  const [data, isLoading, error] = useFetch(
    "http://localhost:9990/projects/" + id
  );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.put(`http://localhost:9990/projects/${id}`, formData);
      navigate("/projects");
    } catch (error) {
